...
Page({
  ...
  async getSuggest(keywords) {
    let suggest = await request("/search/suggest", {
      keywords,
      type: "mobile",
    });
  ...
  async getSearch(content) {
    ...
    let searchResultData = await request("/search", {
      keywords: content,
      limit: 20,
      offset: this.data.pageNum * 20,
      type,
    });
  ...
  toSongDetail(e) {
    wx.navigateTo({
      url:
        "/pages/songDetail/songDetail?song=" +
        e.currentTarget.dataset.musicid +
        "&type=searchList",
    });
  },
  ...
  toMusicList(e) {
    wx.navigateTo({
      url:
        "/pages/musicList/musicList?musiclistid=" +
        e.currentTarget.dataset.musiclistid,
    });
  },
